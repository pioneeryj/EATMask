...
preprocessed_dataset_folder = '/nas_homes/yoonji/medmask/nnUNet_preprocessed/Dataset606_all_TotalSegmentator/nnUNetPlans_3d_fullres'
...
splits_file = '/nas_homes/yoonji/medmask/nnUNet_preprocessed/Dataset606_all_TotalSegmentator/splits_final.json'
...
dataset_json =load_json('/nas_homes/yoonji/medmask/nnUNet_preprocessed/Dataset606_all_TotalSegmentator/dataset.json')
### Your nnUNet plans json
plans = load_json('/nas_homes/yoonji/medmask/nnUNet_preprocessed/Dataset606_all_TotalSegmentator/nnUNetPlans.json')
...
for idx in range(max_calibration_samples):
    ...
    for class_idx in range(num_classes):
        ...
        if isinstance(target, (list, tuple)):
            # Handle deep supervision - use the first (highest resolution) target
    ...
    del inp, logits, mean_softmax, target
    if idx % 5 == 0:
...
for class_idx in range(num_classes):
    all_probs_per_class[class_idx] = np.concatenate(all_probs_per_class[class_idx])
    all_labels_per_class[class_idx] = np.concatenate(all_labels_per_class[class_idx])
