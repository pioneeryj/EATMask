...
import argparse
from metric_evaluation_final import center_crop, pad_to_target_size, calculate_dice_score, calculate_nsd_score
...
if __name__ == '__main__':
    ...
    idx=0
    # Collect probabilities and labels for calibration per class
    num_classes = args.num_classes
    # Initialize calibration accumulators once (across all files)
    n_bins = args.calib_bins if hasattr(args, 'calib_bins') else 10
    calib_accum = {
        'sum_pred': [np.zeros(n_bins, dtype=np.float64) for _ in range(num_classes)],
        'sum_true': [np.zeros(n_bins, dtype=np.float64) for _ in range(num_classes)],
        'count': [np.zeros(n_bins, dtype=np.int64) for _ in range(num_classes)]
    }
    ...
    for file in file_path:
        ...
        # Collect calibration data using logits/probabilities and labels
        # prob shape from predictor is (C, H, W, D). Ensure label_ matches size
        if label_.shape != prob.shape[1:]:
            # resize label to match prob spatial dims
            label_resized = F.interpolate(label_.unsqueeze(0).unsqueeze(0).float(),
                                          size=prob.shape[1:],
                                          mode='nearest').squeeze(0).squeeze(0).long()
        else:
            label_resized = label_

        probs_tensor = torch.from_numpy(prob).float() if isinstance(prob, np.ndarray) else prob.float()
        # Incremental calibration binning to avoid huge memory usage
        for c in range(num_classes):
            class_probs = probs_tensor[c, ...].detach().cpu().numpy().ravel()
            binary_labels = (label_resized == c).detach().cpu().numpy().ravel().astype(np.int64)
            # filter out non-finite probabilities to avoid NaNs in bincount sums
            finite_mask = np.isfinite(class_probs)
            if not np.any(finite_mask):
                continue
            class_probs = class_probs[finite_mask]
            binary_labels = binary_labels[finite_mask]
            # clip to [0,1] for safety
            np.clip(class_probs, 0.0, 1.0, out=class_probs)
            # compute bin indices
            inds = np.clip(np.digitize(class_probs, bin_edges[1:-1], right=False), 0, n_bins - 1)
            # accumulate per bin
            # use bincount per bin; do separately for probs and labels and counts
            bc_count = np.bincount(inds, minlength=n_bins)
            bc_sum_pred = np.bincount(inds, weights=class_probs, minlength=n_bins)
            bc_sum_true = np.bincount(inds, weights=binary_labels, minlength=n_bins)
            calib_accum['count'][c] += bc_count
            calib_accum['sum_pred'][c] += bc_sum_pred
            calib_accum['sum_true'][c] += bc_sum_true
        # free up GPU cache between cases
        ...
        del img, props, label, label_
        del seg, prob, class_indices, pred_one_hot
    ...
    # ===== Calibration Curves (memory-efficient) =====
    print("\nGenerating calibration curves (incremental binning)...")
    n_bins = calib_accum['count'][0].shape[0]
    # Use a single default color (blue) for all classes
    default_color = 'C0'

    # Fixed grid for 105 classes: (5, 21) for wider layout
    fig_rows, fig_cols = 7, 15
    fig, axes = plt.subplots(fig_rows, fig_cols, figsize=(fig_cols * 3.5, fig_rows * 3))
    axes = axes.flatten()
    ece_per_class = []

    # compute per-class reliability
    for c in range(num_classes):
        count = calib_accum['count'][c]
        sum_pred = calib_accum['sum_pred'][c]
        sum_true = calib_accum['sum_true'][c]
        valid = count > 0
        if not np.any(valid):
            ece_per_class.append(np.nan)
            if c < len(axes):
                axes[c].text(0.5, 0.5, f'Class {c}\nNo samples', ha='center', va='center', transform=axes[c].transAxes)
                axes[c].set_xlim([0,1]); axes[c].set_ylim([0,1])
            continue
        conf = np.zeros_like(sum_pred)
        acc = np.zeros_like(sum_true)
        conf[valid] = sum_pred[valid] / count[valid]
        acc[valid] = sum_true[valid] / count[valid]
        # ECE weighted by bin frequency (use only valid bins in denominator to avoid divide-by-zero)
        denom = np.sum(count[valid])
        ece = np.sum(np.abs(acc[valid] - conf[valid]) * (count[valid] / denom)) if denom > 0 else np.nan
        ece_per_class.append(ece)
        if c < len(axes):
            ax = axes[c]
            # x: confidence (bin means), y: accuracy
            ax.plot(conf[valid], acc[valid], marker='o', color=default_color)
            ax.plot([0,1],[0,1],'--',color='gray',linewidth=1)
            ax.set_title(f'Class {c} (ECE: {ece:.4f})')
            ax.set_xlim([0,1]); ax.set_ylim([0,1]); ax.grid(alpha=0.3)

    plt.tight_layout()
    calib_path = f'./calibration_curve_per_class_{args.model}_new.png'
    plt.savefig(calib_path, dpi=150, bbox_inches='tight'); plt.close(fig)
    print(f"Saved per-class calibration curves to {calib_path}")

    # Aggregated curve from combined bins
    total_count = np.sum([np.sum(calib_accum['count'][c]) for c in range(num_classes)])
    if total_count > 0:
        sum_pred_agg = np.sum([calib_accum['sum_pred'][c] for c in range(num_classes)], axis=0)
        sum_true_agg = np.sum([calib_accum['sum_true'][c] for c in range(num_classes)], axis=0)
        count_agg = np.sum([calib_accum['count'][c] for c in range(num_classes)], axis=0)
        valid = count_agg > 0
        conf_agg = np.zeros_like(sum_pred_agg)
        acc_agg = np.zeros_like(sum_true_agg)
        conf_agg[valid] = sum_pred_agg[valid] / count_agg[valid]
        acc_agg[valid] = sum_true_agg[valid] / count_agg[valid]
        denom = np.sum(count_agg[valid])
        ece_agg = np.sum(np.abs(acc_agg[valid] - conf_agg[valid]) * (count_agg[valid] / denom)) if denom > 0 else np.nan
        plt.figure(figsize=(8,8))
        plt.plot(conf_agg[valid], acc_agg[valid], marker='o', linewidth=2)
        plt.plot([0,1],[0,1],'--',color='gray',linewidth=2)
        plt.xlim([0,1]); plt.ylim([0,1]); plt.grid(alpha=0.3)
        agg_path = join(args.checkpoint, args.model, f'calibration_curve_aggregated_{args.model}_new.png')
        plt.tight_layout(); plt.savefig(agg_path, dpi=150, bbox_inches='tight'); plt.close()
        print(f"Saved aggregated calibration curve to {agg_path} (ECE: {ece_agg:.4f})")
    else:
        print("Skipped aggregated calibration curve due to empty data")