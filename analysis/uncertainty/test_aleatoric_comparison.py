'''
- train 데이터 불러오기
- simpleunet pretrained weight 불러와서 kendalgal aleatoric uncertainty map 측정
- entropy based aleatoric uncertainty map 측정
'''
...
# ===== SimpleUNet (STUNet보다 parameter가 훨씬 적음) =====
...
def _main_impl():
    ...
    # ===== 모델 구축 =====
    print("\n🧠 Building SimpleUNet with Variance Head...")

    # SimpleUNet backbone (STUNet보다 훨씬 적은 parameter)
    # base_channels=32 -> 약 2-3M parameters
    unet = SimpleUNet3D(in_channels=1, num_classes=105, base_channels=32)

    # Add variance head on top of SimpleUNet
        ...
        for train_iter in pbar:
            try:
                ...
                if epoch == 0 and train_iter == 0:
                    print(f"[DEBUG] Training target type: {type(target)}", flush=True)
                    if isinstance(target, (list, tuple)):
                        print(f"[DEBUG] Target is list/tuple with {len(target)} elements", flush=True)
                        for i, t in enumerate(target):
                            print(f"[DEBUG]   [{i}] shape: {t.shape}, dtype: {t.dtype}", flush=True)
                    else:
                        print(f"[DEBUG] Target shape: {target.shape}, dtype: {target.dtype}", flush=True)

                # Handle deep supervision: target이 list인 경우 마지막 원소만 사용 (full resolution)
                if isinstance(target, (list, tuple)):
                    target = target[-1]
        ...
        with torch.no_grad():
            for val_iter in range(iters_val):
                try:
                    ...
                    # DEBUG: Print target type and shape to identify the issue
                    if val_iter == 0:  # Only log first validation batch for debugging
                        print(f"[DEBUG] Validation target type: {type(target)}", flush=True)
                        if isinstance(target, (list, tuple)):
                            print(f"[DEBUG] Target is list/tuple with {len(target)} elements", flush=True)
                            for i, t in enumerate(target):
                                print(f"[DEBUG]   [{i}] shape: {t.shape}, dtype: {t.dtype}", flush=True)
                    ...
                    # Handle deep supervision: target이 list인 경우 마지막 원소만 사용 (full resolution)
                    if isinstance(target, (list, tuple)):
                        print(f"[DEBUG] Converting list target to single tensor (using last element)", flush=True)
                        target = target[-1]
