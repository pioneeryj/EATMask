...
# 이어서 돌리기 체크포인트 (best.pth)
## medmask ##
# totalseg
# checkpoint_file = "/nas_homes/yoonji/medmask/nnUNet_results/Dataset606_all_TotalSegmentator/STUNetTrainer__nnUNetPlans__3d_fullres/medmask_0.6_1000epoch/checkpoint_best.pth"
# amos
# checkpoint_file = "/nas_homes/yoonji/medmask/nnUNet_results/Dataset219_AMOS2022_postChallenge_task2/STUNetTrainer__nnUNetPlans__3d_fullres/medmask_0.6_1000epoch/checkpoint_best.pth"

## spark ##
checkpoint_file = "/nas_homes/yoonji/medmask/nnUNet_results/Dataset219_AMOS2022_postChallenge_task2/STUNetTrainer__nnUNetPlans__3d_fullres/spark_1000epoch/checkpoint_best.pth"
...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    if stunet_trainer is None:
        raise RuntimeError(f'Could not find requested nnunet trainer {trainer_name} in '
                           ...
                           f'{join(nnunetv2.__path__[0], "training", "STUNetTrainer")}). If it is located somewhere '
                           f'else, please move it there.')
    assert issubclass(stunet_trainer, STUNetTrainer), 'The requested nnunet trainer class must inherit from ' \
                                                    'nnUNetTrainer'
...
def maybe_load_checkpoint(nnunet_trainer: STUNetTrainer, continue_training: bool, validation_only: bool,
    ...
    if continue_training:
        ...
        if pretrained_weights_file is not None:
            ...
            # load_stunet_pretrained_weights(nnunet_trainer.network, pretrained_weights_file, verbose=True)
            # load_stunet_for_autopet(nnunet_trainer.network, pretrained_weights_file, verbose=True)
            load_stunet_ssl_weights(nnunet_trainer.network, pretrained_weights_file, verbose=True)
            # load_stunet_moco_weights(nnunet_trainer.network, pretrained_weights_file, verbose=True)
            # load_swin_weights(nnunet_trainer.network, pretrained_weights_file, verbose=True)
            # load_unimiss_weights(nnunet_trainer.network, pretrained_weights_file, verbose=True)
...
def run_ddp(rank, dataset_name_or_id, configuration, fold, tr, p, use_compressed, disable_checkpointing, c, val,
    ...
    torch.cuda.set_device(torch.device('cuda:0', dist.get_rank()))
...
def run_training_entry():
    ...
    parser.add_argument('-tr', type=str, required=False, default='UNetTrainer',
