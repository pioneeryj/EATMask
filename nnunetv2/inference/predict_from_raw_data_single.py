...
if __name__ == '__main__':
    ...
    parser.add_argument('--inp', default=input_dir)
    parser.add_argument('--checkpoint', default=checkpoint_dir)
    parser.add_argument('--model', default=model_name)
    parser.add_argument('--inp_label', default=label_dir)
    parser.add_argument('--num_classes', default=105, type=int)
    ...
    # logging
    log_filename = f"{args.model}.txt"  # 또는 f"result_{args.model}.txt"
    results_dir = args.results_dir  # 디렉토리 경로
    log_path = join(results_dir, log_filename)  # 파일 경로
    ...
    for file in file_path:
        ...
        dice_score = calculate_dice_score(ret, label_, args.num_classes)
        nsd_score = calculate_nsd_score(ret,label_, args.num_classes)
        idx+=1
        print(idx,":",dice_score)
        print(idx,":",nsd_score)
        dice_scores.append(dice_score)
        nsd_scores.append(nsd_score)

    avg_dice = np.nanmean(dice_scores)
    avg_nsd = np.nanmean(nsd_scores)
    std_dice =np.nanstd(dice_scores)
    std_nsd = np.nanstd(nsd_scores)
    print(f"\n🔥 Average Dice Score: {avg_dice:.4f} ± {std_dice:.4f}")
    print(f"\n🔥 Average NSD Score: {avg_nsd:.4f} ± {std_nsd:.4f}")
