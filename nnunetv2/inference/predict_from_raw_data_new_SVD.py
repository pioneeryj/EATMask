...
if __name__ == '__main__':
    ...
    idx=0
    def get_foreground_mask_from_input(input_img, target_shape):
        # input_img: (1, D, H, W) numpy, target_shape: (H, W, D) or (C,H,W,D)
        if isinstance(input_img, np.ndarray):
            img_t = torch.from_numpy(input_img).float()
        else:
            img_t = input_img.float()
        if img_t.dim() == 4 and img_t.shape[0] == 1:
            img_t = img_t  # (1, D,H,W)
        else:
            img_t = img_t.unsqueeze(0)
        input_mask = (torch.abs(img_t) > 1e-5).float()  # (1, D,H,W)
        # target spatial size
        if len(target_shape) == 4:
            size = target_shape[1:]
        else:
            size = target_shape
        mask_resized = F.interpolate(input_mask.unsqueeze(0), size=size, mode='nearest').squeeze(0)
        # (1, d,h,w) -> (d,h,w) boolean
        return (mask_resized.squeeze(0) > 0.5)

    def compute_svd_on_probs(probs, fg_mask):
        # probs: (C, H, W, D) torch or numpy
        probs_t = torch.from_numpy(probs).float() if isinstance(probs, np.ndarray) else probs.float()
        C, H, W, D = probs_t.shape
        # select foreground pixels
        x = probs_t.permute(1,2,3,0)  # (H,W,D,C)
        x_fg = x[fg_mask]  # (N_fg, C)
        if x_fg.numel() == 0:
            return None, None
        # center data
        x_mean = x_fg.mean(dim=0, keepdim=True)
        x_centered = x_fg - x_mean
        # SVD
        U, S, Vh = torch.linalg.svd(x_centered, full_matrices=False)
        # explained variance ratio
        var = (S**2)
        evr = (var / var.sum()).cpu().numpy()
        S_np = S.cpu().numpy()
        return S_np, evr

    def plot_svd(S, evr, out_path_prefix):
        plt.figure(figsize=(12,5))
        # singular values
        plt.subplot(1,2,1)
        plt.plot(S, marker='o')
        plt.title('Singular Values (foreground probs)')
        plt.xlabel('index'); plt.ylabel('value'); plt.grid(alpha=0.3)
        # explained variance ratio
        plt.subplot(1,2,2)
        plt.plot(evr, marker='o')
        plt.title('Explained Variance Ratio (foreground probs)')
        plt.xlabel('index'); plt.ylabel('ratio'); plt.grid(alpha=0.3)
        plt.tight_layout()
        plt.savefig(out_path_prefix + '_svd.png', dpi=150)
        plt.close()
    ...
    for file in file_path:
        ...
        # ===== Foreground SVD analysis =====
        # build foreground mask from input image resized to prob spatial size
        fg_mask = get_foreground_mask_from_input(img.squeeze(0), prob.shape[1:])  # (H,W,D)
        S_vals, evr = compute_svd_on_probs(prob, fg_mask)
        if S_vals is not None:
            svd_dir = join(args.checkpoint, args.model, 'svd')
            maybe_mkdir_p(svd_dir)
            out_prefix = join(svd_dir, os.path.basename(file).replace(args.model, '').replace('.nii.gz',''))
            plot_svd(S_vals, evr, out_prefix)
