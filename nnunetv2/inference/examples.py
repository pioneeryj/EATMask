if __name__ == '__main__':
    ...
    predictor.initialize_from_trained_model_folder(
        ...
        use_folds=(0,),
        checkpoint_name='/home/yoonji/AnatoMask/Anatomask_results/Dataset601_Total/Pretraining/0218/anatomask/anatomask_checkpoint_epoch_{i+1}.pt',
    )
    # variant 1: give input and output folders
    predictor.predict_from_files(join(nnUNet_raw, 'Dataset601_organs_TotalSegmentator/imagesTs'),
                                 join(nnUNet_raw, 'Dataset601_organs_TotalSegmentator/imagesTs_predlowres'),
                                 save_probabilities=False, overwrite=False,
                                 num_processes_preprocessing=2, num_processes_segmentation_export=2,
                                 folder_with_segs_from_prev_stage=None, num_parts=1, part_id=0)

    # variant 2, use list of files as inputs. Note how we use nested lists!!!
    indir = join(nnUNet_raw, 'Dataset601_organs_TotalSegmentator/imagesTs')
    outdir = join(nnUNet_raw, 'Dataset601_organs_TotalSegmentator/imagesTs_predlowres')
    predictor.predict_from_files([[join(indir, 'TotalSegmentator_0040_0000.nii.gz')],
                                  [join(indir, 'TotalSegmentator_0291_0000.nii.gz')]],
                                 [join(outdir, 'TotalSegmentator_0423_0000.nii.gz'),
                                  join(outdir, 'TotalSegmentator_0561_0000.nii.gz')],
    ...
    indir = join(nnUNet_raw, 'Dataset601_organs_TotalSegmentator/imagesTs')
    predicted_segmentations = predictor.predict_from_files([[join(indir, 'TotalSegmentator_0291_0000.nii.gz')],
                                                            [join(indir, 'TotalSegmentator_0561_0000.nii.gz')]],
                                                           None,
    ...
    img, props = SimpleITKIO().read_images([join(nnUNet_raw, 'Dataset601_organs_TotalSegmentator/imagesTs/TotalSegmentator_0308_0000.nii.gz')])
    img2, props2 = SimpleITKIO().read_images([join(nnUNet_raw, 'Dataset601_organs_TotalSegmentator/imagesTs/TotalSegmentator_0311_0000.nii.gz')])
    img3, props3 = SimpleITKIO().read_images([join(nnUNet_raw, 'Dataset601_organs_TotalSegmentator/imagesTs/TotalSegmentator_0450_0000.nii.gz')])
    img4, props4 = SimpleITKIO().read_images([join(nnUNet_raw, 'Dataset601_organs_TotalSegmentator/imagesTs/TotalSegmentator_0470_0000.nii.gz')])
    ...
    img, props = SimpleITKIO().read_images([join(nnUNet_raw, 'Dataset601_organs_TotalSegmentator/imagesTs/TotalSegmentator_0470_0000.nii.gz')])
