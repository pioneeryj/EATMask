...
class Decoder_forward(nn.Module):
    def __init__(self, dims, num_classes, pool_op_kernel_sizes, conv_kernel_sizes, depth):
        ...
        for u in range(len(pool_op_kernel_sizes)):
        ...
        for u in range(len(pool_op_kernel_sizes)):
            stage = nn.Sequential(
                BasicResBlock(dims[-2 - u] * 2, dims[-2 - u], conv_kernel_sizes[-2 - u], [i // 2 for i in conv_kernel_sizes[-2 - u]], use_1x1conv=True),
                *[BasicResBlock(dims[-2 - u], dims[-2 - u], conv_kernel_sizes[-2 - u], [i // 2 for i in conv_kernel_sizes[-2 - u]]) for _ in range(depth[-2 - u] - 1)]
            )
        ...
        self.upscale_logits_ops = [lambda x: x for _ in range(len(pool_op_kernel_sizes) - 1)]
