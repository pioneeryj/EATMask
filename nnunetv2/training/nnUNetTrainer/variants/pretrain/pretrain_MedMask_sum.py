...
from AnatoMask_sum import SparK, monte_carlo, calculate_softmax, calculate_conditional_entropy,calculate_epistemic_uncertainty, calculate_aleatoric_uncertainty
...
# shape: 4,1,7,7,8
        ...
        # Sum the outputs from epistemic and aleatoric uncertainty streams
        union_output_list = [teacher_ep_output[i] + teacher_al_output[i] for i in range(len(teacher_ep_output))]
        
        # Apply projection head to each feature level
        proj_union_output = []
        for i in range(len(union_output_list)):
            proj_union_output.append(model.module.projection_head[i](union_output_list[i]))
