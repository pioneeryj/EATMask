...
projection_params = list(model.module.projection_head.parameters())
...
################################## load 하는 부분 ############################################################################
# 어디서 불러올지
file_name = f"{args.model_name}_checkpoint_epoch_600.pt"
...
if os.path.exists(checkpoint_path):
    ...
    # Local ddp requires parameter name with prefix 'module.'
    new_state_dict ={}
...
for i in range(start_epoch,epoch):
    ...
    for idx in tqdm(range(iters_train), desc=f"Epoch {i} training", leave=False):
...
# shape: 4,1,7,7,8
        ...
        # mask_union_uncertainty = model.module.mask_uncertainty(batch_size, device, epistemic_map, aleatoric_map, args.mask_ratio) 
        mask_epistemic_intensity = model.module.mask_intensity(batch_size, device, uncertainty_map=epistemic_map, intensity=args.intensity, masking_ratio=args.mask_ratio)
        # mask_aleatoric_intensity = model.module.mask_intensity(batch_size, device, uncertainty_map=aleatoric_map, intensity=args.intensity, masking_ratio=args.mask_ratio)

        ### add for ablation(single uncertainty)
        mask_single_uncertainty = model.module.mask_single_uncertainty(batch_size, device, epistemic_map, args.mask_ratio) 
        ...
        # union_output_list = [torch.cat([teacher_ep_output[i], teacher_al_output[i]], dim=1) for i in range(len(teacher_ep_output))]     
        # proj_union_output = [model.module.projection_head[i](union_output_list[i])
                            # for i in range(len(union_output_list))]
        
        single_output_list = [teacher_ep_output[i] for i in range(len(teacher_ep_output))]
        proj_single_output = [model.module.projection_head[i](single_output_list[i]) # changed projection head input shape at Anatomask.py
                            for i in range(len(single_output_list))]
        ...
        # student_output = model.module.forward_encoder(inp,active_b1ff=mask_union_uncertainty)
        student_single_output = model.module.forward_encoder(inp,active_b1ff=mask_single_uncertainty)

        # student reconstruction loss
        # loss_p,_ = model.module.forward_loss(inp_s, rec_s, mask_union_uncertainty)
        loss_p_single,_ = model.module.forward_loss(inp_single, rec_single, mask_single_uncertainty)
        # feature level loss    
        # cos_loss = loss_cosine(proj_union_output, student_output)
        cos_loss_single = loss_cosine(proj_single_output, student_single_output)
        

        ## change to single for ablataion
        loss = w_cos * cos_loss_single + w_p*loss_p_single
        ...
        loss_p_value = loss_p_single.item()
        #mae_loss_value = mae_loss.item()
        cos_loss_value = cos_loss_single.item()
    ...
    if i == 0 or 'ema_loss' not in locals():
    ...
    if (i + 1) % 100 == 0:
        epoch_checkpoint_path = join(output_folder,args.model_name + f'_checkpoint_epoch_{i+1}.pt' )
