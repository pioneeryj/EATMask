...
# checkpoint_path = '/home/yoonji/AnatoMask/Anatomask_results/Dataset601_Total/Pretraining/0218/anatomask/anatomask_head_latest.pt'

input_dir = "/nas_homes/yoonji/medmask/nnUNet_raw/Dataset606_all_TotalSegmentator/imagesTs"
checkpoint_dir = "/home/yoonji/AnatoMask/Anatomask_results/MedMask/Dataset606_all_TotalSegmentator/STUNetTrainer__nnUNetPlans__3d_fullres"
...
## 이거 사용할 것임
    def initialize_from_trained_STUNet(self, model_training_output_dir: str,
        ...
        num_input_channels = determine_num_input_channels(plans_manager, configuration_manager, dataset_json)
        #
        network = STUNet(num_input_channels, 105, depth=[1] * 6, dims=[32 * x for x in [1, 2, 4, 8, 16, 16]],
                      pool_op_kernel_sizes=[[2,2,2],[2,2,2],[2,2,2],[2,2,2],[1,1,1]], conv_kernel_sizes=[[3, 3, 3]] * 6, enable_deep_supervision=False)
        # network = STUNet(num_input_channels, 105, depth=[2] * 6, dims=[64 * x for x in [1, 2, 4, 8, 16, 16]],
        #               pool_op_kernel_sizes=[[2,2,2],[2,2,2],[2,2,2],[2,2,2],[1,1,1]], conv_kernel_sizes=[[3, 3, 3]] * 6, enable_deep_supervision=False)
        # network = STUNet(num_input_channels, 105, depth=[3] * 6, dims=[96 * x for x in [1, 2, 4, 8, 16, 16]],
        #               pool_op_kernel_sizes=[[2,2,2],[2,2,2],[2,2,2],[2,2,2],[1,1,1]], conv_kernel_sizes=[[3, 3, 3]] * 6, enable_deep_supervision=False)

        # from nnunetv2.training.nnUNetTrainer.STUNetTrainer import STUNet
        # network = STUNet(num_input_channels, 105, depth=[3] * 6, dims=[96 * x for x in [1, 2, 4, 8, 16, 16]],
        #                  pool_op_kernel_sizes=[[2, 2, 2], [2, 2, 2], [2, 2, 2], [2, 2, 2], [1, 1, 1]],
        #                  conv_kernel_sizes=[[3, 3, 3]] * 6, enable_deep_supervision=False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--inp', default="/nas_homes/yoonji/medmask/nnUNet_raw/Dataset606_all_TotalSegmentator/imagesTs")
    parser.add_argument('--checkpoint', default="/home/yoonji/AnatoMask/Anatomask_results/MedMask/Dataset606_all_TotalSegmentator/STUNetTrainer__nnUNetPlans__3d_fullres")
    parser.add_argument('--model', default="anatomask")
    ...
    predictor.predict_from_files(join(nnUNet_raw, 'Dataset606_all_TotalSegmentator/imagesTs'),
                                 join(nnUNet_raw,'Dataset606_all_TotalSegmentator/imagesTs_pred',model_name), # output folder
    ...
    # img, props = SimpleITKIO().read_images(['/nas_homes/yoonji/medmask/nnUNet_raw/Dataset601_organs_TotalSegmentator/imagesTr/TotalSegmentator_0361_0000.nii.gz'])
