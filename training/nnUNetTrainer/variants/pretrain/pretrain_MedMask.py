...
# checkpoint_path = join(output_folder, model_name + '_0218.pt')
...
pretrained_model = "/home/yoonji/AnatoMask/large_ep4k.model"  # pretrained weights 경로
...
for i in range(start_epoch,epoch):
    ...
    for idx in range(iters_train):
        ...
        logit_list = monte_carlo(model=model_dropout, inp=inp, T=10)
        ...
        mask_union_uncertainty = model.module.mask_uncertainty(batch_size, device, epistemic_map, aleatoric_map) 
        mask_epistemic_intensity = model.module.mask_intensity(batch_size, device, uncertainty_map=epistemic_map)
        # shape: 4,1,7,7,8
        mask_aleatoric_intensity = model.module.mask_intensity(batch_size, device, uncertainty_map=aleatoric_map)
        ...
        union_output_list = [torch.cat([teacher_ep_output[i].to(device), teacher_al_output[i].to(device)], dim=1) for i in range(len(teacher_ep_output))]     
        
        if not hasattr(model, 'projection_head'):
            projection_layers =[]
            for ep, al in zip(teacher_ep_output, teacher_al_output):
                in_features = ep.shape[1] + al.shape[1]
                out_features = ep.shape[1]
                projection_layers.append(nn.Conv3d(in_features, out_features, kernel_size=1).to(device))
            model.projection_head = nn.ModuleList(projection_layers)
            optimizer.add_param_group({'params': model.projection_head.parameters()})    

        proj_union_output = [model.projection_head[i](union_output_list[i].to(device))
        ...
        # mae loss btw student and teacher rec
        # mae_loss = ((rec_t - rec_s)**2).mean(dim=2, keepdim=False)
